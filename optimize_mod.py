...
def main_fss(bypass):
    ...
    for index in range(SCHOOL.school_size):
        
        fish = SCHOOL.school[index].pos
        
        print(fish)
        
        print(msg.SIM_ID+global_.simulationID)
        
        simulate.create_sim_file(fish, 0, index)
        
        if bypass != "-bypass":
            simulate.run_simulation_hfss(global_.HFSS_RUN_STRINGS)
        
        s11,s21,s31,s41, amp_imbalance = simulate.read_simulation_results(0,index)
        fit, data_to_store, derivative_array = pso.fitness([s11,s21,s31,s41, amp_imbalance],[0,index])
        
        SCHOOL.school[index].cost = fit 
        ...
        db_manager.save_data_to_plot(data_to_plot,0,index)
    ...
    data_to_store={
        ...
        "gbest": SCHOOL.best_fish.cost,
        #"best_particle_id":best_index,
        "best_particle": json.dumps(SCHOOL.best_fish.pos),
        "iteration":0
    }
...
def set_school():
    school  = fss.FSS(global_.n_variables, global_.var_min, global_.var_max, 100, global_.n_particulas)
    school.init_school()
    logging.info(msg.PARTICLES_CREATED)
    return school

def iterations_school(ite, bypass):
    

    for i in range(ite):
        print(msg.NUM_ITERATIONS+str(iter))
        print("current it.:"+str(i))
    
        
        SCHOOL.individual_movement(simulador,i,bypass)
        SCHOOL.update_best_fish()
        SCHOOL.feeding()
        SCHOOL.collective_instinctive_movement()
        SCHOOL.collective_volitive_movement(simulador,i,bypass, db_manager)
        SCHOOL.update_steps(i)
        SCHOOL.update_best_fish()
    ...
    print("Minimo global encontrado: "+str(SCHOOL.best_fish.pos))

def simulador(pos,i,id, bypass):
    
    simulate.create_sim_file(pos,i+1, id)
    
    if bypass!="-bypass":
        simulate.run_simulation_hfss(global_.HFSS_RUN_STRINGS)
    s11,s21,s31,s41, amp_imbalance = simulate.read_simulation_results(i+1,id)
    fit, data_to_store,derivative_array = pso.fitness([s11,s21,s31,s41,amp_imbalance],[i+1,id])
    
    return fit, data_to_store,derivative_array
